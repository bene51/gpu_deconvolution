...
void deconvolveGPU(FILE **dataFiles, FILE *resultFile, int dataW, int dataH, int dataD, float **h_Kernel, int kernelH, int kernelW, int nViews, int iterations)
{
	...
	*h_ResultGPU;
	...
	int datasize = dataW * dataH * sizeof(float);
	...
	*fftPlanInv;
	...
	prepareKernelsGPU(h_Kernel, kernelH, kernelW, fftH, fftW, nViews, d_KernelSpectrum, d_KernelHatSpectrum, fftPlanFwd[0]);
	...
	int *planes = (int *)malloc(dataD * sizeof(int));
	for(z = 0; z < dataD; z++)
		planes[z] = z;
...
//			}
			...
			checkCudaErrors(cudaMemcpyAsync(
					...
					streams[stream]));
			// checkCudaErrors(cudaStreamAddCallback(streams[stream], callback, planes + z + stream, 0));
			printf("bla %d\n", stream);

			// load from HDD
			if(z > 0 || stream == 3) {
				int streamToWait = (stream + 1) % nStreams;
				printf("z %d (stream %d): waiting for stream %d\n", z, stream, streamToWait);
				checkCudaErrors(cudaStreamSynchronize(streams[streamToWait]));
				fwrite(h_Data[0] + streamToWait * datasize, sizeof(float), dataW * dataH, resultFile);
				for(v = 0; v < nViews; v++) {
					fread(h_Data[v] + streamToWait * datasize, sizeof(float), dataW * dataH, dataFiles[v]);
					normalizeMinMax(h_Data[v] + streamToWait * datasize, dataW * dataH, 40, 62000);
				}
			}
		}
	}
